...
void CSystemTray::MinimiseToTrayOnStartup(CWnd* pWnd)
{
#ifndef _WIN32_WCE
...
CSystemTray::~CSystemTray()
{
	...
	// ADDED BY GEERT VAN HORRIK * BEGIN
	if (::IsWindow(m_wndInvisible.m_hWnd))
		m_wndInvisible.DestroyWindow(); 
	// ADDED BY GEERT VAN HORRIK * END
    DestroyWindow();
}
...
void CSystemTray::AddMenuTitle(CMenu* popup, LPCTSTR lpszTitle)
{
    // insert a separator item at the top
    popup->InsertMenu(0, MF_BYPOSITION | MF_SEPARATOR);
...
//
void CSystemTray::DrawPopupMenuTitle(LPDRAWITEMSTRUCT lpDi)
{
	USES_CONVERSION;
    ...
    if (font)
    {
        ...
        DrawText(lpDi->hDC, A2W(lpszMenuTitle), -1, &lpDi->rcItem, DT_SINGLELINE|DT_VCENTER|DT_LEFT);
